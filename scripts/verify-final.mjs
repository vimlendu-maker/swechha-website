...
export const NOT_FINAL = [
  ...
  { file: 'method.html', why: 'FINISHED \u2014 how a reading is decided, serving at /method. Outside THIS test for the same reason as about.html: it is a finished page and not one of the six situations the final set is about. It is the one page that STATES the rules the rest of the set obeys \u2014 what counts as a reading, the kinds of limit, the four closed vocabularies, the refusals \u2014 and its own gates re-check two of those claims against the built situation index and the built forest-fire page, so it cannot describe a site that has stopped behaving that way.' },
  ...
  { file: 'teach.html', why: 'FINISHED \u2014 the Teach index, serving at /teach, and it IS the section\u2019s directory rather than a landing page with a list somewhere else: /learn already merges the two at 30 items in 8 categories and this is 43 in 8, so a second index would be a second hand-maintained list of the same sessions. Outside THIS test for the reason learn.html is. Its own gates live in scripts/build-teach.mjs, and the load-bearing one is that the directory is DERIVED \u2014 the index must link every session the same walk wrote, or the build refuses.' },
  { file: 'teach/', why: 'FINISHED \u2014 Swechha\u2019s Bridge the Gap compendium: one page per theme and one per session from data/teach/, serving at /teach/<theme> and /teach/<theme>/<session>, plus the teacher\u2019s guide at /teach/before-you-start and the 334-term glossary at /teach/a-to-z. Published as eight THEMES rather than as the 321-page manual it arrived as. Three gates carry this section. THE ABSENT ROW IS THE DESIGN: the manual fills its own template unevenly (a teaching sequence in 36 of 43 sessions, a stated time in only 10), so a metadata row renders only where the manual filled the field \u2014 no placeholder, no em dash, no greyed row \u2014 and a rendered row with an empty body stops the build. THE BLANK SCREEN IS ONLY WHERE THERE IS NO PHOTOGRAPH: three themes have no honest frame and get the halftone screen over paper-2 reproducing nothing, and a theme that has a photograph but renders the blank screen fails, because the moment it appears next to a usable frame it is decoration rather than a statement. AND SIX TITLES ARE OURS, NOT THE MANUAL\u2019S: chapter 1\u2019s first six activities are untitled in the source, so their pages must say the name is ours \u2014 a title of ours passed off as the manual\u2019s is the only place this section could mislead. A session page also carries no photograph, no page script and its own print rules, because a teacher prints it and carries it into a room.' },
