...
/* ═══ HERO CROSS-CHECK (moved from build-hero.mjs, Option B, AD-47) ════════
   The hero slide figures must match the situation page figures. This check
   runs AFTER all builds complete, so it compares fresh data against fresh
   pages, eliminating the build-order timing conflict that used to cause daily
   failures when build:hero ran before build:situations.
   ═══════════════════════════════════════════════════════════════════════ */
{
  const HERO_CHECKS = [
    {
      id: 'h-air', route: '/now/air', file: 'situation-air.html',
      regex: /id="air-aqi"[^>]*>([0-9,]+)/,
      getExpected: () => String(J('air-delhi.json').city_reading.aqi),
      label: '/now/air',
    },
  ];

  console.log('\nHERO CROSS-CHECK — slides vs situation pages (post-build validation)');
  for (const check of HERO_CHECKS) {
    const path = join(V3, check.file);
    if (!existsSync(path)) {
      console.log(`  FAIL ${check.id.padEnd(10)} ${check.file} not found`);
      fail++;
      continue;
    }
    const html = readFileSync(path, 'utf8');
    ...
    if (!m) {
      console.log(`  FAIL ${check.id.padEnd(10)} could not extract figure from ${check.file}`);
      fail++;
      continue;
    }
    const onPage = String(m[1]).replace(/,/g, '');
    const expected = check.getExpected();
    if (onPage !== expected) {
      console.log(`  FAIL ${check.id.padEnd(10)} says ${onPage}, dataset says ${expected} — mismatch`);
      fail++;
    } else {
      console.log(`  ok   ${check.id.padEnd(10)} agrees with ${check.label} at ${onPage}`);
    }
  }
}
