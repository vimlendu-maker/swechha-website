...
## A-44.11 · Addendum, later the same day: the Vercel prediction came true, and the CA pin

A-44.7 said that if Vercel's undici rejected the CAAQMS chain, the route
would degrade to the mirror as an ordinary fallback. It deployed, and that is
exactly what happened: `/api/air` on Vercel failed the plain-fetch CAAQMS
attempt on the cross-signed eMudhra intermediate, `served_by` honestly named
the mirror — and the route served the 02:00 observation under a page showing
12:00. Honest, and still the inversion A-44.5 exists to end.

**The fix, measured before it was written**: hand the chain the server itself
serves to `node:https` as explicit trust anchors via the `ca` option. From a
machine where plain fetch fails identically to Vercel: **200 OK in 80ms**.
With explicit anchors there is no path-building ambiguity left to trip on.

What changed:

- **`certs/cpcb-caaqms-chain.pem`** — the server's own PUBLIC chain, six
  certificates (leaf + cross-signed eMudhra intermediate + Comodo AAA root +
  the emSign-side trio). Nothing secret: these are the bytes every visitor is
  handed. `certs/README.md` carries the regeneration one-liner and the expiry
  table — the leaf rotates **20 Sep 2026**, which is fine by design: `ca`
  needs the *issuers* trusted, not the leaf, so a renewed leaf under the same
  intermediates keeps validating.
- **`lib/cpcb-ca.ts`** — the same bytes embedded as a string constant,
  because a runtime `readFileSync` would depend on Next's output file tracing
  including `certs/` in the deployed function; a static import is bundled
  unconditionally. Generated by `scripts/lib/regen-cpcb-ca.mjs`, pinned
  byte-for-byte to the PEM by `lib/cpcb-ca.test.ts` — the transcription
  convention's warranty, again.
- **`fetchDelhiLive` is now a three-rung ladder**: CA-pinned CAAQMS
  (`caFetchText`, the rung that works on Vercel) → plain-fetch CAAQMS (free
  to try; wins the day CPCB serves a chain undici accepts) → the mirror.
  Both CAAQMS rungs pass the same gates; **any failure on any rung —
  including a rotted CA bundle when CPCB re-keys — is an ordinary fallback**,
  never an error response. The tests force it: a valid-but-wrong trust
  anchor must reject (an *unparseable* one will not do — Node quietly
  ignores garbage `ca` input, which is itself worth knowing), and the ladder
  must then serve the mirror and say so.
- **`/api/ward` and `/api/ward/subscribe` join the ladder** — the routes
  serving per-reader nearest-monitor data and validating subscription
  station names, where freshness matters most of all. Same rows shape,
  contracts unchanged; `/api/ward` now also names `served_by`.

> **A pinned trust anchor is allowed to rot, and the code must already know
> that.** When CPCB re-keys under a new CA, the pinned rung starts failing,
> the ladder degrades to exactly the pre-pin behaviour, and `served_by`
> shows it — that visible degradation, not an alert or an error, is the
> signal to re-run the regeneration command in certs/README.md.
